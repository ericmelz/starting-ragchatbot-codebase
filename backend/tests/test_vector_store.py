...
class TestVectorStore:
    ...
    def test_search_max_results_comparison(
        self, temp_chroma_path, sample_course, sample_course_chunks
    ):
        """Compare search results with different MAX_RESULTS values"""

        # Create two vector stores with different MAX_RESULTS
        store_with_results = VectorStore(
            temp_chroma_path + "_good", "all-MiniLM-L6-v2", max_results=5
        )
        store_zero_results = VectorStore(
            temp_chroma_path + "_bad", "all-MiniLM-L6-v2", max_results=0
        )
    ...
    def test_search_with_invalid_course_name(self, populated_vector_store):
        ...
        results = populated_vector_store.search(
            "testing", course_name="Nonexistent Course"
        )
    ...
    def test_search_with_combined_filters(self, populated_vector_store):
        ...
        results = populated_vector_store.search(
            "testing", course_name="Test Course", lesson_number=2
        )
...
class TestVectorStoreConfiguration:
    ...
    def test_initialization_with_different_max_results(self, temp_chroma_path):
        ...
        store_normal = VectorStore(
            temp_chroma_path + "_normal", "all-MiniLM-L6-v2", max_results=5
        )
        assert store_normal.max_results == 5

        # Test with zero (the problematic value)
        store_zero = VectorStore(
            temp_chroma_path + "_zero", "all-MiniLM-L6-v2", max_results=0
        )
        assert store_zero.max_results == 0

        # Test with large value
        store_large = VectorStore(
            temp_chroma_path + "_large", "all-MiniLM-L6-v2", max_results=100
        )
