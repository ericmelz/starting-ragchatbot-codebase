...
class TestRAGSystemQueryFlow:
    ...
    def test_query_with_existing_session(self, rag_system):
        ...
        response, sources = rag.query("Follow up question", session_id="existing_session")
        
        # Should get conversation history
        rag.session_manager.get_conversation_history.assert_called_once_with("existing_session")
...
class TestRAGSystemErrorHandling:
    ...
    def test_query_session_manager_error(self, rag_system):
        ...
        rag.session_manager.get_conversation_history.side_effect = Exception("Session error")
        rag.mock_ai_generator.generate_response.return_value = "Response"
...
class TestRAGSystemCourseManagement:
    ...
    def test_add_course_document(self, rag_system, sample_course):
        ...
        rag.document_processor.process_course_document.return_value = (sample_course, [])
        ...
        with patch('os.path.exists') as mock_exists:
            ...
            rag.document_processor.process_course_document.assert_called_once_with("/fake/path/course.txt")
            
            # Should add to vector store
            rag.mock_vector_store.add_course_metadata.assert_called_once_with(sample_course)
    ...
    def test_add_course_folder(self, rag_system):
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            mock_exists.return_value = True
            mock_listdir.return_value = ["course1.txt", "course2.txt", "not_a_course.jpg"]
